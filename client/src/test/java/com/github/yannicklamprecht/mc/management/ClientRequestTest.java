...
class ClientRequestTest {
    ...
    @Test
    void systemMessageOmitsReceivingPlayersToBroadcastToEveryone() throws Exception {
        // Per the MSMP spec, receivingPlayers defaults to "all players" only when the field is
        // absent from the payload entirely; an explicit empty array means "these zero players"
        // and reaches nobody. See MinecraftManagementSession.SystemMessageView.
        List<JsonNode> requests = new ArrayList<>();
        try (var client = recordingClient(requests)) {
            client.session().sendSystemMessage("hello");
            assertEquals(1, requests.size());
