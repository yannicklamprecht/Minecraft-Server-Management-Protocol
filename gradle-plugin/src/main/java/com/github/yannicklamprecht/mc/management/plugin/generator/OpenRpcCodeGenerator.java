package com.github.yannicklamprecht.mc.management.plugin.generator;
...
public final class OpenRpcCodeGenerator {
    /**
     * Name of the JSON file, written by the schema extraction task into the schemas directory,
     * mapping each Minecraft version to the MSMP protocol version extracted from it. Read here to
     * generate {@code ProtocolVersions}.
     */
    public static final String MINECRAFT_VERSION_MAPPING_FILE_NAME = "minecraft-version-protocol-map.json";
    ...
    public OpenRpcCodeGenerator(String clientClassName) {
        if (clientClassName == null || clientClassName.isBlank()) {
            clientClassName = "com.github.yannicklamprecht.mc.management.transport.MinecraftManagementClient";
        }
    ...
    public void generateAll(Path schemaRoot, Path output, String basePackage) throws IOException {
        ...
        try (var versions = Files.list(schemaRoot)) {
            for (Path versionDir : versions.filter(Files::isDirectory).sorted().toList()) {
                ...
                generate(schema, version, output, basePackage);
                protocolVersions.add(version);
            }
        }

        generateProtocolVersions(output, basePackage, protocolVersions, readMinecraftVersionMapping(schemaRoot));
    }

    private Map<String, String> readMinecraftVersionMapping(Path schemaRoot) throws IOException {
        Path mappingFile = schemaRoot.resolve(MINECRAFT_VERSION_MAPPING_FILE_NAME);
        if (!Files.exists(mappingFile)) return Map.of();
        return MAPPER.readValue(mappingFile.toFile(), new TypeReference<LinkedHashMap<String, String>>() {});
    }

    /**
     * Generates {@code ProtocolVersions}, a top-level class (in {@code basePackage}) mapping every
     * known Minecraft version to the MSMP protocol version it speaks, plus the set of supported
     * protocol versions and the latest one. Rerun {@code generateMinecraftManagementSources} (after
     * {@code extractMinecraftManagementSchemas}) to refresh it as new Minecraft/protocol versions
     * ship - no hand-maintained mapping needed.
     */
    private void generateProtocolVersions(Path output, String basePackage, List<String> protocolVersions,
                                           Map<String, String> minecraftVersionToProtocol) throws IOException {
        if (protocolVersions.isEmpty()) return;

        List<String> sortedProtocolVersions = protocolVersions.stream()
                .sorted(OpenRpcCodeGenerator::compareVersions)
                .toList();
        String latest = sortedProtocolVersions.get(sortedProtocolVersions.size() - 1);

        TypeName stringType = ClassName.get(String.class);
        TypeName listOfString = ParameterizedTypeName.get(ClassName.get(List.class), stringType);
        TypeName mapOfStringString = ParameterizedTypeName.get(ClassName.get(Map.class), stringType, stringType);
        TypeName optionalOfString = ParameterizedTypeName.get(ClassName.get(Optional.class), stringType);

        CodeBlock.Builder supportedInit = CodeBlock.builder().add("$T.of(", List.class);
        for (int i = 0; i < sortedProtocolVersions.size(); i++) {
            if (i > 0) supportedInit.add(", ");
            supportedInit.add("$S", sortedProtocolVersions.get(i));
        }
        supportedInit.add(")");

        CodeBlock.Builder staticBlock = CodeBlock.builder();
        staticBlock.addStatement("$T map = new $T<>()",
                ParameterizedTypeName.get(ClassName.get(LinkedHashMap.class), stringType, stringType), LinkedHashMap.class);
        for (var entry : minecraftVersionToProtocol.entrySet()) {
            staticBlock.addStatement("map.put($S, $S)", entry.getKey(), entry.getValue());
        }
        staticBlock.addStatement("MINECRAFT_VERSION_TO_PROTOCOL_VERSION = $T.unmodifiableMap(map)", Collections.class);

        TypeSpec protocolVersionsType = TypeSpec.classBuilder("ProtocolVersions")
                .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                .addJavadoc("Generated mapping between Minecraft versions and Minecraft Server Management Protocol\n"
                ...
                .addField(FieldSpec.builder(listOfString, "SUPPORTED_PROTOCOL_VERSIONS", Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
                        .initializer(supportedInit.build())
                        .build())
                .addField(FieldSpec.builder(String.class, "LATEST_PROTOCOL_VERSION", Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
                        .initializer("$S", latest)
                        .build())
                .addField(FieldSpec.builder(mapOfStringString, "MINECRAFT_VERSION_TO_PROTOCOL_VERSION", Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
                        .addJavadoc("Keyed by Minecraft version (e.g. the server status response's {@code version.name}).\n")
                        .build())
                .addStaticBlock(staticBlock.build())
                .addMethod(MethodSpec.constructorBuilder().addModifiers(Modifier.PRIVATE).build())
                .addMethod(MethodSpec.methodBuilder("protocolVersionForMinecraftVersion")
                        .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                        .returns(optionalOfString)
                        .addParameter(String.class, "minecraftVersion")
                        .addJavadoc("Looks up the MSMP protocol version spoken by a given Minecraft version (e.g. from\n"
                        ...
                        .addStatement("return $T.ofNullable(MINECRAFT_VERSION_TO_PROTOCOL_VERSION.get(minecraftVersion))", Optional.class)
                        .build())
                .build();

        JavaFile.builder(basePackage, protocolVersionsType)
                .indent("    ")
                .build()
                .writeTo(output);
    }
